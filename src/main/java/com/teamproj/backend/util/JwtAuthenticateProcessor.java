...
public class JwtAuthenticateProcessor {
    ...
    public UserDetailsImpl forceLogin(String token){
        /*
            token 이 존재함 : Authorization 헤더에 값이 있음
            token 이 존재하는데 그 정보가 유효하지 않음 : 비회원으로 처리.
         */

        // token 이 유효하지 않을 경우 빈문자열("")로 변환함.
        token = ValidChecker.tokenCheck(token);
    ...
    // jwt 토큰 정보를 발급받을 때마다 무조건 사용자 정보를 받아야 하는 상황에서 벗어나기 위해 getUser 기능을 userDetailsImpl 에서 외부로 빼낸 것.
